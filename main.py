...
from domain import Igraci, Timovi, Kuglane,Bodovi_igraca_,Rezultati_timova
...
@app.route('/igraci', methods=['GET', 'POST','PUT'])
def handle_igraci():
...
@app.route('/kuglane', methods=['GET', 'POST','PUT'])
def handle_kuglane():
    if request.method == 'GET':
        kuglane = Kuglane.listaj()
        return jsonify({"kuglane": kuglane})
    elif request.method == 'PUT':
        status, greske = Kuglane.update(request.get_json())
    elif request.method == 'POST':
        status, greske = Kuglane.dodaj(request.get_json())
        if status:
            return Response(status=201)
        else:
            r = Response(status=500)
