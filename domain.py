from model import Tim, Igrac, Kuglana, Bodovi_igraca,Rezultati
...
class Kuglane:
    ...
    def listaj():
        ...
        q = select(s for s in Kuglana)

        def dohvati_veze(x):
            if "timovi" in x:
                x["timovi"] = [Tim[y].to_dict() for y in x["timovi"]]
            return x
        kuglane = [dohvati_veze(s.to_dict(with_collections=True)) for s in q]
        return kuglane
